...
-- look at "Create table" column for the code to re-create the address table
...
GROUP BY film.title;
-- count(actor_id) shows the number of actor in each film
...
SELECT x.first_name, x.last_name, x.email, x.country  
FROM 
(
...
-- 8c. You find that you no longer need the view top_five_genres. Write a query to delete it.x 
